...
def as_question(text: str) -> str:
    """מסיים כותרת של שאלה בסימן שאלה.

    כותרת כמו "מותר לרסס בושם בשבת" נקראת כקביעה ולא כשאלה, ובעמוד
    הנושא היא יושבת ליד כותרות שכן מסתיימות בסימן שאלה — כך שההבדל
    נראה כמו חוסר עקביות ולא כמו סגנון. הפונקציה אידמפוטנטית, ולכן
    אפשר להריץ אותה שוב על אותו קובץ בלי לצבור סימנים.

    ``slugify`` מסירה פיסוק, ולכן התוספת אינה משנה את ה-slug של השאלה
    ואינה שוברת כתובת קיימת.
    """
    text = (text or "").strip()
    if not text:
        return text
    # נקודה בסוף כותרת שאלה היא טעות הקלדה ולא סוף משפט, ולכן מוחלפת
    # ולא נשמרת לצד סימן השאלה.
    text = text.rstrip(" .־-")
    if text.endswith("?"):
        return text
    return text + "?"
