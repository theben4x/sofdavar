...
class Config:
    ...
    #: הביטוי שמודגש בזהב בתוך שורות הפתיחה — ההבטחה שבגללה נשארים כאן.
    #: עובד כמו ``VERSE_EMPHASIS``, דרך המסנן ``emphasize``, ולכן הוא חייב
    #: להופיע ב-``HOME_LEDE`` בדיוק כך. ביטוי אחד ולא שניים: שתי הדגשות
    #: בפסקה של שלוש שורות מבטלות זו את זו.
    HOME_LEDE_EMPHASIS = ("תשובה ישירה במשפט אחד",)
