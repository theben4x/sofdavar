...
class Config:
    ...
    #:
    #: כבוי כי הוא לא הוכיח את עצמו. מדידה על 13 שאילתות אמיתיות
    #: (``scripts/eval_retrieval.py --source owner``): החיפוש הלקסיקלי
    #: לבדו מגיע ל-76.9% במקום הראשון, והוספת השכבה הסמנטית לא שיפרה
    #: אף אחת מארבע השאלות שהוא פספס. על שאילתות סינתטיות היא כן הוסיפה
    #: כיסוי — ולכן היא נשארת מותקנת ולא נמחקת, ותיבחן מחדש כשיהיו יותר
    #: שאלות במאגר.
    #:
    #: כשהוא כבוי, ``numpy`` והמודל אינם נטענים כלל — הבדיקה קודמת
    #: לייבוא ב-``db.suggestions``, כך שהכיבוי חוסך גם זמן עלייה.
